...
for(var i = 0; i < array.length; i++){
	//running calculateSTI on whole array, not array[i]
...
function calculateSTI(array){
  ...
  if(bonus > 0.13){
    bonus = 0.13; // no adjustment for negative bonus
  } else if (bonus < 0) {
		bonus = 0;
	}

  newArray[1] = bonus; // percentage of STI employee to receive
	// adjusted annual compensation ADDED ROUNDING TO 2 DECIMALS
  newArray[2] = +((baseSalary * (1.0 + bonus)).toFixed(2));
	//total bonus rounded to nearest dollar BUT IT ISN'T ROUNDED
...
function getBaseSTI(reviewScore){
  ...
  return basePercent; //this shouldn't be - 1
}
